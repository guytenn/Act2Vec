...
from environments.painter_gym import PainterGym
import gensim
...
class AADDPG(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, action_buff_size, act2vec_model, k=10, gamma=0.99, memory_policy=None, eval_env=None, nb_train_steps=10,
        ...
        super(AADDPG, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose, policy_base=DDPGPolicy,
                                   requires_vec_env=False)

        # Parameters.
        self.act2vec_model = act2vec_model
        self.index = gensim.similarities.MatrixSimilarity(gensim.matutils.Dense2Corpus(act2vec_model.wv.vectors.T))
        self.embedding_dim = self.act2vec_model.wv.vector_size
        self.k = k
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                rootUbuntu = "/home/guy/"
                root = rootUbuntu
                path = os.path.join(root, "Dropbox (MLGroup)/research/Action2Vec/samples/banana.ndjson")

                stroke_length = len(list(self.act2vec_model.wv.vocab)[0])
                DL = PainterDataLoader(path, max_size=1000, stroke_length=stroke_length)
                args = get_args()
                # image_res = [256, 256]
                state_res = [self.action_buff_size, self.embedding_dim, 1]

                # with tf.variable_scope("reward_network", reuse=False):
                # disc_step = tf.get_variable(
                #     "disc_step", [], tf.int32,
                #     initializer=tf.constant_initializer(0, dtype=tf.int32),
                #     trainable=False)
                with tf.device('/device:GPU:1'):
                    self.reward_network = RewardNetwork(args=args, step=None,
                                                        image_shape=state_res,
                                                        batch_size=1,
                                                        norm_fn=norm, scope_name='disc',
                                                        real_images=DL.get_images_as_embeddings(inds=range(200),
                                                                                                buffer_size=self.action_buff_size,
                                                                                                act2vec_model=self.act2vec_model)
                                                        )

                self.reward_network.set_session(self.sess)

                for env in self.env.venv.envs:
                    if isinstance(env, PainterGym):
                        env.set_reward_net(self.reward_network)
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    ### AADDPG addition ###
                    ########################
                    self.best_nearest_action = tf.placeholder(tf.float32,
    ...
    def _get_best_nearest_actions(self, actions, obs, k=10):
        start = timer()
        # get similarity scores for all actions in vocabulary
        similarities = self.index[actions * np.abs(self.action_space.low)]
        # get k nearest neighbors
        topk_inds = similarities.argsort()[:, -k:]
        candidates = self.act2vec_model.wv.vectors[topk_inds] / np.abs(self.action_space.low)
        # find the q_values for each of the neighbors
        feed_dict = {self.obs_train: np.repeat(obs, k, axis=0),
                     self.actions: candidates.reshape([-1, self.embedding_dim])}
        q_values = self.sess.run(fetches=self.normalized_critic_tf,
                                 feed_dict=feed_dict).reshape([-1, k])
        # for each entry of neighbors, find the neighbor with the best q value
        best_actions_inds = np.argmax(q_values, axis=1)
        chosen_actions = np.zeros((len(actions), self.embedding_dim))
        for i in range(len(chosen_actions)):
            chosen_actions[i] = candidates[i, best_actions_inds[i], :]

        end = timer()
        print("{0}-nearest neighbors took {1} seconds".format(k, end - start))
    ...
    def _setup_actor_optimizer(self):
        ...
        self.actor_loss = tf.reduce_mean((self.best_nearest_action - self.actor_tf)**2)
    ...
    def _train_step(self, step, writer, log=False, percentage=0.0):
        """
        ...
        td_map = {
            ...
            self.best_nearest_action: self._get_best_nearest_actions(batch['actions'], batch['obs0'], k=self.k)
        }
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        for t_train in range(self.nb_train_steps):
                            ...
                            critic_loss, actor_loss = self._train_step(step, writer, log=t_train == 0, percentage=t_train/float(self.nb_train_steps))
