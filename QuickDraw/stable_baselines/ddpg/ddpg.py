...
class DDPG(OffPolicyRLModel):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                DL = PainterDataLoader(path, max_size=500, stroke_length=5)
                args = get_args()
                image_res = [256, 256]
                ...
                with tf.device('/device:GPU:1'):
                    self.reward_network = RewardNetwork(args=args, step=None,
                                                        image_shape=[image_res[0], image_res[1], 1],
                                                        batch_size=1,
                                                        norm_fn=norm, scope_name='disc',
                                                        real_images=DL.get_images(range(10), image_res[0], image_res[1])
                                                        )
    ...
    def _train_step(self, step, writer, log=False):
        ...
        ## Reward Network ##
        ####################
        self.reward_network.backward(np.expand_dims(batch['obs0'][-1], axis=0))
