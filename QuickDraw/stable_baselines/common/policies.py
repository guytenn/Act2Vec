...
from stable_baselines.a2c.utils import conv, conv1, linear, conv_to_fc, batch_to_seq, seq_to_batch, lstm
from stable_baselines.common.distributions import make_proba_dist_type
from stable_baselines.common.input import observation_input

from utils.components import LSTM

def nature_cnn(scaled_images, reuse, **kwargs):
    """
    ...
    """
    # activ = tf.tanh
    # layer = LSTM(scaled_images[0],
    #              outputDim=64,
    #              BiDirectional=True,
    #              onlyLastOutput=True,
    #              seqLength=scaled_images[1],
    #              reuse=reuse)
    # return activ(linear(layer, 'fc1', n_hidden=64, init_scale=np.sqrt(2)))

    activ = tf.nn.relu

    layer_1 = activ(conv1(scaled_images[0], 'c1', n_filters=32, filter_size=8, stride=4, init_scale=np.sqrt(2), **kwargs))
    layer_2 = activ(conv1(layer_1, 'c2', n_filters=64, filter_size=4, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_3 = activ(conv1(layer_2, 'c3', n_filters=64, filter_size=3, stride=1, init_scale=np.sqrt(2), **kwargs))
...
class BasePolicy(ABC):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, scale=False,
        ...
        with tf.variable_scope("input", reuse=False):
            ...
            self.seq_len_ph = tf.placeholder(tf.int32, [n_batch], name="seq_len")
            self.action_ph = None
            if add_action_ph:
...
class LstmPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, layers=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                extracted_features = cnn_extractor((self.processed_obs, self.processed_obs_len), **kwargs)
            else:
                activ = tf.tanh
    ...
    def step(self, obs, state=None, seq_len=None, mask=None, deterministic=False):
        if deterministic:
            return self.sess.run([self.deterministic_action, self._value, self.snew, self.neglogp],
                                 {self.obs_ph: obs, self.states_ph: state, self.masks_ph: mask, self.seq_len_ph:seq_len})
        else:
            return self.sess.run([self.action, self._value, self.snew, self.neglogp],
                                 {self.obs_ph: obs, self.states_ph: state, self.masks_ph: mask, self.seq_len_ph:seq_len})

    def proba_step(self, obs, state=None, seq_len=None, mask=None):
        return self.sess.run(self.policy_proba, {self.obs_ph: obs, self.states_ph: state, self.masks_ph: mask, self.seq_len_ph:seq_len})

    def value(self, obs, state=None, seq_len=None, mask=None):
        return self.sess.run(self._value, {self.obs_ph: obs, self.states_ph: state, self.masks_ph: mask, self.seq_len_ph:seq_len})
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, seq_len=None, reuse=False, layers=None, net_arch=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                pi_latent = vf_latent = cnn_extractor((self.processed_obs, self.seq_len_ph), reuse, **kwargs)
            else:
    ...
    def step(self, obs, state=None, seq_len=0, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp = self.sess.run([self.deterministic_action, self._value, self.neglogp],
                                                   {self.obs_ph: obs, self.seq_len_ph: seq_len})
        else:
            action, value, neglogp = self.sess.run([self.action, self._value, self.neglogp],
                                                   {self.obs_ph: obs, self.seq_len_ph: seq_len})
        return action, value, self.initial_state, neglogp

    def proba_step(self, obs, state=None, seq_len=0, mask=None):
        return self.sess.run(self.policy_proba, {self.obs_ph: obs, self.seq_len_ph: seq_len})

    def value(self, obs, state=None, seq_len=0, mask=None):
        return self.sess.run(self._value, {self.obs_ph: obs, self.seq_len_ph: seq_len})
...
class CnnPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, seq_len=None, reuse=False, **_kwargs):
        super(CnnPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, seq_len, reuse,
