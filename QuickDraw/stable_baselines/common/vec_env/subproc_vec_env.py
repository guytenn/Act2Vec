...
def _worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        try:
            ...
            if cmd == 'step':
                observation, seq_len, reward, done, info = env.step(data)
                if done:
                    observation, seq_len = env.reset()
                remote.send((observation, seq_len, reward, done, info))
            elif cmd == 'reset':
                observation, seq_len = env.reset()
                remote.send((observation, seq_len))
            elif cmd == 'render':
                remote.send(env.render(*data[0], **data[1]))
            elif cmd =='random_with_seed':
                seed = np.random.randint(100000)
                env.random_with_seed(seed)
            elif cmd == 'close':
                remote.close()
                break
            elif cmd == 'get_spaces':
...
class SubprocVecEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        self.waiting = False
        obs, seq_lens, rews, dones, infos = zip(*results)
        return np.stack(obs), np.stack(seq_lens), np.stack(rews), np.stack(dones), infos

    def random_with_seed(self):
        for remote in self.remotes:
            remote.send(('random_with_seed', None))

    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        obs, seq_lens = zip(*results)
        return np.stack(obs), np.stack(seq_lens)
