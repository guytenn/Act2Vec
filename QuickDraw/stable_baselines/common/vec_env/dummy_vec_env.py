...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        self.buf_seq_len = np.zeros((self.num_envs,), dtype=np.int32)
    ...
    def step_wait(self):
        for env_idx in range(self.num_envs):
            obs, self.buf_seq_len[env_idx], self.buf_rews[env_idx], self.buf_dones[env_idx], self.buf_infos[env_idx] =\
                self.envs[env_idx].step(self.actions[env_idx])
            if self.buf_dones[env_idx]:
                obs, self.buf_seq_len[env_idx] = self.envs[env_idx].reset()
            self._save_obs(env_idx, obs)
        return (np.copy(self._obs_from_buf()), np.copy(self.buf_seq_len), np.copy(self.buf_rews), np.copy(self.buf_dones),
                self.buf_infos.copy())
    ...
    def reset(self):
        for env_idx in range(self.num_envs):
            obs, self.buf_seq_len[env_idx] = self.envs[env_idx].reset()
            self._save_obs(env_idx, obs)
        return np.copy(self._obs_from_buf()), np.copy(self.buf_seq_len)
